...
Local maintenance and seeding utilities. `seed-board.mjs` can bulk-create fake people on a board and optionally connect them to the root node. `seed-demo-user.mjs` can populate one user's board with a repeatable demo cluster that includes people, notes, AI summaries, and connections. `generate-linkedin-connections.mjs` creates large LinkedIn-compatible CSV and ZIP import fixtures.

### `fixtures/`

Generated test fixtures that are useful for local performance checks. `fixtures/linkedin/` contains large LinkedIn import files generated by `npm run generate:linkedin-fixture`.
...
- unsigned browser-local graph state for editing before login
...
- circular pie-zone group rendering with proportional color slices and thin orbit guides
...
- editable browser-local board state before sign-in
...
- blob groups render as circular pie zones whose color slices follow the merged member colors and arrange members inside their slice
