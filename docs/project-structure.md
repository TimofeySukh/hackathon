...
Generated test fixtures that are useful for local performance checks. `fixtures/linkedin/` contains large LinkedIn import files generated by `npm run generate:linkedin-fixture` and `linkedin_archive_sample_10_contacts.zip`, the source archive mirrored by the starter sample graph.
