...
- The `/embed` application uses the real production origin and therefore shares the normal
  ...
  stays inside the frame. Google authentication opens in a popup only when embedded. Its
  allowlisted callback carries `sdn_auth_popup=google`, and the top-level callback window
  closes only after Supabase restores an authenticated session. The standalone site keeps
  its existing full-page redirect.
