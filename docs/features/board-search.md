...
- Feature-specific layout/motion: collapsed→open is a spring width transition; the
  ...
  width. The mobile placeholder is `Search` for every session; wider authenticated layouts
  retain `Ask AI to find someone…`. The first-run LinkedIn URL hint uses `primary-container`
