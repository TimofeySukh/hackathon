...
The Flutter prototype uses `InteractiveViewer` for normal board pan and zoom. Real working people icons are painted by a single screen-space canvas layer with painter hit testing for tap and drag, rather than one Flutter widget per person or a large transformed world canvas. Dense stress icons are also painted outside the transformed world stack in screen coordinates, using the `TransformationController` matrix only to convert world positions to viewport positions. This keeps dense icon layers constrained to the screen-sized paint surface. The stress renderer stores synthetic node positions in typed arrays, indexes them into spatial grid buckets, culls by viewport, draws avatar icons from a preprocessed sprite with `drawAtlas`, and draws optional synthetic edges with raw line segments instead of per-edge paths. Real and synthetic icon sizes are zoom-aware with a readable lower bound, and dense edges use level-of-detail sampling when zoomed out or actively navigating. Flower shapes normalize amplitude by radius so the same setting stays visually usable across large circles and small people icons.
