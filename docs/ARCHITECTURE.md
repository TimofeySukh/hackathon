...
- `flutter_board/lib/main.dart` contains the Flutter circle graph prototype. Flutter widgets own normal circles, real people, controls, and inspector UI, while a screen-space `CustomPaint` overlay owns dense synthetic stress icons and their optional edges.
...
The Flutter prototype uses `InteractiveViewer` for normal board pan and zoom. Real working people icons are painted by a single world-space canvas layer with painter hit testing for tap and drag, rather than one Flutter widget per person. Dense stress icons are intentionally painted outside the transformed world stack in screen coordinates, using the `TransformationController` matrix only to convert world positions to viewport positions. This keeps the stress layer constrained to the screen-sized paint surface and avoids thousands of Flutter widgets for synthetic icons. The stress renderer stores synthetic node positions in typed arrays, indexes them into spatial grid buckets, culls by viewport, draws avatar icons from a preprocessed sprite with `drawAtlas`, and draws optional synthetic edges with raw line segments instead of per-edge paths. Flower shapes normalize amplitude by radius so the same setting stays visually usable across large circles and small people icons.
