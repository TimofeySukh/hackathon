...
- Mitigation (2026-06-11): the real-entity harness confirmed ~700 people / 45 circles dropped the board to ~2 FPS. Addressed by viewport culling + level-of-detail in the render passes — only nodes inside the visible world rect (padded by `CULL_OVERSCAN`) are rendered, and individual people are dropped below `PEOPLE_LOD_SCALE`. Measured live DOM set stays ~120–150 nodes deep inside a dense field (vs 700+ total) and drops to circles-only when zoomed out, so frame cost is bounded by screen area rather than total node count. The FPS meter was extracted into its own component so its tick no longer re-renders the whole tree. See the design log for the rationale.
