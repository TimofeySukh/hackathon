...
- create people with a board double-click or double-tap, form circular pie-zone blob groups by dropping one person near another or touching an existing blob area, and drag the middle of a blob area to move the whole blob group
...
22. Drop one person near another, confirm a circular filled blob group appears, then merge people or groups and confirm the circle becomes a proportional pie zone by member category, with thin orbit guides and no transparent ring gaps. Confirm same-name categories share one slice even if they came from different groups, while different categories remain separate even when their colors match. Drop a person near a grouped member but outside the blob area and confirm it does not join the group. Drag the middle of the blob area and confirm the whole group moves together.
