...
This repository contains a minimal React, Vite, and TypeScript folder graph viewer.

The current product is intentionally narrow:

- a left sidebar with selectable graph folders
- a full-window graph surface
- mouse drag navigation and cursor-centered zoom
- a dark/light theme toggle
- a CSS-rendered point grid with motion-triggered point highlights
- compact graph nodes and SVG edges
- Supabase-backed Google login
- one personal graph-space board record per signed-in user
- no graph editing, graph content persistence, filters, search, or collaboration yet
...
- `src/App.tsx`: current folder graph behavior.
...
- `src/index.css`: current graph and account visual system.
...
- Keep the minimal graph interaction stable while product scope is finalized.
